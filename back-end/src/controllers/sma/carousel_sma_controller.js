...
const getAllCarousel = async (req, res) => {
    try {
        const [rows] = await db.execute("SELECT * FROM carousel_sma");
...
const addCarousel = async (req, res) => {
    try {
        ...
        await db.execute(
            "INSERT INTO carousel_sma (judul, gambar, created_at) VALUES (?, ?, NOW())",
            [judul, gambar]
        );
...
const updateCarousel = async (req, res) => {
    try {
        ...
        let query = "UPDATE carousel_sma SET judul=?";
        let values = [judul];
...
const deleteCarousel = async (req, res) => {
    try {
        const { id } = req.params;
        await db.execute("DELETE FROM carousel_sma WHERE id = ?", [id]);
