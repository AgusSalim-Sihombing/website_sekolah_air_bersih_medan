...
const getAllSchedule = async (req, res) => {
    try {
        const [rows] = await pool.execute("SELECT * FROM schedule_admin_sma ORDER BY event_date");
...
const getScheduleById = async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.execute("SELECT * FROM schedule_admin_sma WHERE id = ?", [id]);
...
const createSchedule = async (req, res) => {
    try {
        ...
        await pool.execute(
            "INSERT INTO schedule_admin_sma (title, description, event_date) VALUES (?, ?, ?)",
...
const updateSchedule = async (req, res) => {
    try {
        ...
        await pool.execute(
            "UPDATE schedule_admin_sma SET title = ?, description = ?, event_date = ? WHERE id = ?",
...
const deleteSchedule = async (req, res) => {
    try {
        const { id } = req.params;
        await pool.execute("DELETE FROM schedule_admin_sma WHERE id = ?", [id]);
